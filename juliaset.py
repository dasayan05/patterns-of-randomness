...
if __name__ == '__main__':
    import cmath
    r = 0.7506812389423355

    count = 0
    for theta in np.arange(0., cmath.pi * 2, 0.05):
        C = r * (cmath.e ** (1j * theta))
        mapping = partial(douady_hubbard_polynomial, c = C)
        image = julia_set(mapping,
                          min_coordinate=-1.5 - 1j,
                          max_coordinate=1.5 + 1j,
                          width=800, height=600)
        plt.axis('off')
        plt.imshow(image, cmap=plt.cm.twilight_shifted, origin='lower')
        plt.savefig(f'junk/{count}.png', bbox_inches='tight', inches=0, dpi=300)
        count += 1